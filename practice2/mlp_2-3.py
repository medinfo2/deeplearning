...
class MLP(object):
	...
	def confusion_matrix(self, x_data, y_data, gpu=-1):
		if gpu >= 0:
			x_data = cuda.to_gpu(x_data, device=gpu)
			y_data = cuda.to_gpu(y_data, device=gpu)
		# x = Variable(x_data)
		y = self._forward(x_data)
		if y is None:
			logging.error(
				'cannot calculate confusion '
				'matrix due to the forward implementation.'
			)
			return None
		prob = F.softmax(y).data
		pred_y = []
		for p, y in zip(prob, y_data):
			pre_y = numpy.argmax(p)
			#print pre_y,y
			pred_y.append(pre_y)
		pred_y = numpy.array(pred_y)
		return metrics.confusion_matrix(y_data, pred_y)

	@classmethod
	def print_confusion_matrix(cls, matrix, logger=None, border=True):
		ptable = PrettyTable(
            ["Label{}".format(i) for i in range(matrix.shape[0])]
        )
...
def main(description, gpu, output):
	...
	matrix = mlp.confusion_matrix(data_test, target_test)
	MLP.print_confusion_matrix(matrix)
